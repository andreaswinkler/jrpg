/*
** 2014/02/17
** 2014/07/22 - ECS reconstruction and optimization
**
** This is the GAME class. It represents a game instance. It is static
** because there can only be one game active at any time. Toggling the 
** 'active' (state) property of the class determines if the game loop 
** runs or everything is paused.
**
** The class also handles loading and caching of maps and houses 
** the game loop.
*/
var Game = {
    ...
    **    success [function]: the method that is called after the map 
    **       was successfully loaded either from cache or from the server                                
    */    
    loadMap: function(mapId, success) {
    
        // first we make sure the game is paused, and nothing 
        // happens until the new map is loaded
        this.active = false;
    
        // if there is already a map loaded, we suspend it for later 
        // usage. the Entity Manager returns a storable stack object
        // for us
        if (this.map) {
        
            this.cache[this.map.id] = { 
                map: this.map, 
                stack: EntityManager.suspendStack() 
            };
        
        }
    
        // if we have the requested map already in the cache, we load 
        // it from there. The Entity Manager gets passed the cached 
        // stack object and re-constructs a working version
        // Afterwards the callback function is invoked
        if (this.cache[mapId]) {
        
            this.map = this.cache[mapId].map;
            
            EntityManager.loadSuspendedStack(this.cache[mapId].stack);
            
            success();
        
        }
        // if the map was not cached already we need to load the 
        // map data from the server. We then tell the Entity Manger
        // to process the map description and create a stack of 
        // entities out of it.
        // Once this is finished we load all textures necessary to 
        // display the map and invoke the callback function afterwards
        else {
        
            // make a json post request to the server passing on 
            // the map id. The server returns a json object that 
            // can directly be used as a map
            JRPG.request('map.load', { id: mapId }, function(d) {
            
                this.map = d;
                
                // process all map objects
                EntityManager.loadStackFromMap(this.map.objects);
            
                // load all textures necessary for this map
                TextureSystem.loadMapTextures(this.map, success);    
    ...
    /*
    ** This is the actual game loop. First we process everything by 
    ** letting the Entity Manager loop through all active entities. 
    ** Afterwards we let the UI update all visible parts like game 
    ** canvas, minimap and the user interface itself
    **
    ** Parameters: 
    **    ticks [int]: the milliseconds passed since the last call to 'loop' 
    **
    ** Returns: 
    **    nothing                                          
    */
